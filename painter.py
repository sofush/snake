...
class Painter:
    def __init__(self, board: Board, snake: Snake):
        self.snake = snake
        self.board = board
        self.window = pg.display.set_mode((DEFAULT_SCREEN_WIDTH, DEFAULT_SCREEN_HEIGHT))
        ...
        if state == GameState.PLAYING:
            for x in range(self.board.width):
                for y in range(self.board.height):
                    ...
                    pg.draw.rect(self.window, tile.color, rect)
    ...
    def calculate_tile_size(self) -> (int, int):
        ...
        width = self.window.get_width() - margin_width_total
        height = self.window.get_height() - margin_height_total
